...
export function ContactMobileStickyBar() {
  ...
  useEffect(() => {
    document.body.classList.toggle("contact-bar-visible", visible);
    return () => document.body.classList.remove("contact-bar-visible");
  }, [visible]);
  ...
  return (
    <div
      className="pr-mobile-action-bar fixed inset-x-0 bottom-0 z-50 lg:hidden"
      role="region"
      aria-label="Snelle acties"
      data-contact-mobile-bar
    >
      <div className="pr-mobile-action-bar__inner mx-auto max-w-lg">
        ...
        >
          <Phone size={14} className="shrink-0 text-[#C6A062]" aria-hidden />
          Bel direct
        </a>
        <a
          href="#contact-form"
